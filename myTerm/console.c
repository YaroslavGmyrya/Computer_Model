...
main (int argc, char *argv[])
{
  ...
  int elements_read
      = fread ((void *)big_char, sizeof (int), ROWS * COLS, file);
  ...
  for (int i = 0; i < 7; i++)
    {
      sc_addIOEntry (i, '<',
                     memory[i] < 0 ? memory[i] ^ (1 << 14) : memory[i]);
    }
  ...
  while (1)
    {
      ...
      else if ((value == KEYS_NAME[KEY_T] || value == KEYS_NAME[KEY_t])
               && interactive_mode)
        {
          force_step = 1;
          signal (SIGALRM, IRC);
      ...
      else if ((value == KEYS_NAME[KEY_o] || value == KEYS_NAME[KEY_O])
               && interactive_mode)
        {
          int y = 23, x = 100;
                  ...
                  while (1)
                    {
                      ...
                      else if ((KEYS_NAME[KEY_a] <= value
                                && value <= KEYS_NAME[KEY_f])
                               || (KEYS_NAME[KEY_A] <= value
                                   && value <= KEYS_NAME[KEY_F])
                               || value == KEYS_NAME[KEY_PLUS]
                               || value == KEYS_NAME[KEY_MINUS]
                               || (KEYS_NAME[KEY_0] <= value
                                   && value <= KEYS_NAME[KEY_9])
                               || value == KEYS_NAME[KEY_MORE]
                               || value == KEYS_NAME[KEY_LESS])
                        {

                          if (block == 0 && value != KEYS_NAME[KEY_PLUS]
                              && value != KEYS_NAME[KEY_MINUS]
                              && value != KEYS_NAME[KEY_MORE]
                              && value != KEYS_NAME[KEY_LESS])
                            {
      ...
      else if ((value == KEYS_NAME[KEY_R] || value == KEYS_NAME[KEY_r])
               && interactive_mode)
        {
          interactive_mode = 0;
          sc_regSet (T, 0);
      ...
      else if (value == KEYS_NAME[KEY_i]
               || value == KEYS_NAME[KEY_I] && interactive_mode)
        {
      ...
      else if (value == KEYS_NAME[KEY_S]
               || value == KEYS_NAME[KEY_s] && interactive_mode)
        {
          mt_gotoXY (26, 40);
      ...
      else if (value == KEYS_NAME[KEY_L]
               || value == KEYS_NAME[KEY_l] && interactive_mode)
        {
          int x = 26, y = 40;
          mt_gotoXY (x, y);
          rk_mytermregime (1, 50, 1, 1, 1);
              ...
              else if (value == KEYS_NAME[KEY_ENTER])
                {
                  if (block == 0)
                  ...
                  else
                    {
                      ...
                      if (buffer[0] == '+')
                        sc_commandEncode (0, command_int, operand_int,
                                          &encoded_value);
                      else if (buffer[0] == '-')
                        {
                          sc_commandEncode (1, command_int, operand_int,
                                            &encoded_value);
                        }
          ...
          while (1)
            {
              ...
              if ((KEYS_NAME[KEY_a] <= value && value <= KEYS_NAME[KEY_f])
