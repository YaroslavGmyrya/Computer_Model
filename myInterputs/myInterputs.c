...
int
ALU (int command, int operand, int in_cache)
{
  ...
  switch (command)
    ...
    case 0x0A:
      ...
      mt_gotoXY (y, x);
      printf ("Введите значение, которое Вы хотите записать по адресу %d: ",
              operand);
      mt_gotoXY (++y, x);
      while (1)
        {
...
CU ()
{
  ...
  if (sc_memoryGet (instruction_counter, &instruction) == 0
      && sc_commandDecode (instruction, &sign, &command, &operand) == 0
      && interputs_counter == 0)
    {
